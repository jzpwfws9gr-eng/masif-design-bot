...
# ============================================================
# AGREED PATCH — صور الإحصائيات كاملة + ملف PDF + صفحات تفاصيل الاختيارات
# ============================================================
...
def _stats_active_participants(start_day, end_day):
    ...
    active = [p for p in ranking if totals.get(p, 0) > 0 or stats.get("participation_count", {}).get(p, 0) > 0]
...
def create_day_choices_image(day):
    ...
    for idx, r in enumerate(rows, start=1):
        ...
        cy = y + row_h // 2
        items = [
            (1450, _clean_display_name(r.get("participant")), 240),
            (1215, _clean_display_name(r.get("keeper")), 145),
            (1010, _clean_display_name(r.get("p1")), 145),
            (805, _clean_display_name(r.get("p2")), 145),
            (600, _clean_display_name(r.get("p3")), 145),
            (390, _clean_display_name(r.get("captain")), 145),
            (155, str(int(r.get("total", 0))), 95),
        ]
        for x, txt, mw in items:
            draw_text(draw, (x, cy), txt or "-", get_font(24 if x != 155 else 26), fill="#FFFFFF" if x != 155 else "#FDE68A", max_width=mw)
        y += row_h + 10
...
def create_statistics_pdf(start_day, end_day):
    ...
    first.save(out_path, "PDF", save_all=True, append_images=rest, resolution=120.0)
    for im in images:
        try:
            im.close()
        except Exception:
