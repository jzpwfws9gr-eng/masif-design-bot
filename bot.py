...
# ============================================================
# FANTASY ONLY — clean runtime
# ============================================================
PARTICIPANTS_FILE = "fantasy_participants.json"
...
def load_participants_state():
    ...
    try:
        with open(PARTICIPANTS_FILE, "r", encoding="utf-8") as f:
            data = json.load(f)
        names = [normalize_name(x) for x in (data.get("participants") or []) if normalize_name(x)]
        if names:
...
def save_participants_state():
    with open(PARTICIPANTS_FILE, "w", encoding="utf-8") as f:
        json.dump({"participants": PARTICIPANTS}, f, ensure_ascii=False, indent=2)


def _remove_participant_from_workbook(path, name):
    try:
        wb = load_workbook(path)
        changed = False
        for ws in wb.worksheets:
            for row in range(ws.max_row, 1, -1):
                if normalize_name(ws.cell(row=row, column=1).value) == name:
...
async def add_participant_command(update: Update, context: ContextTypes.DEFAULT_TYPE):
    text = re.sub(r"^/(?:إضافة_متسابق|اضافة_متسابق|إضافة_مشارك|اضافة_مشارك)(?:@\w+)?\s*", "", update.message.text or "").strip()
    name = normalize_name(text)
    if not name:
        await update.message.reply_text("اكتبها كذا:\n/إضافة_متسابق نايف حمود")
        return
    if name in PARTICIPANTS:
        await update.message.reply_text(f"{name} موجود مسبقًا.")
        return
    ...
    for day in get_existing_days(1, 999):
        try:
            ...
            pass
    await update.message.reply_text(f"تمت إضافة {name} ✅\nالعدد الآن: {len(PARTICIPANTS)}")


async def remove_participant_command(update: Update, context: ContextTypes.DEFAULT_TYPE):
    text = re.sub(r"^/(?:حذف_متسابق|حذف_مشارك)(?:@\w+)?\s*", "", update.message.text or "").strip()
    name = normalize_name(text)
    if not name:
        await update.message.reply_text("اكتبها كذا:\n/حذف_متسابق نايف حمود")
        return
    if name not in PARTICIPANTS:
        await update.message.reply_text("الاسم غير موجود في قائمة المشاركين.")
        return
...
async def participants_list_command(update: Update, context: ContextTypes.DEFAULT_TYPE):
    lines = [f"مشاركو فانتزي المصيف ({len(PARTICIPANTS)}):", ""]
    lines.extend(f"{i}. {name}" for i, name in enumerate(PARTICIPANTS, 1))
    await update.message.reply_text("\n".join(lines))


async def start(update: Update, context: ContextTypes.DEFAULT_TYPE):
    await update.message.reply_text(
        "🏆 بوت فانتزي المصيف 2026\n\n"
        "التشكيلات والنتائج:\n"
        "/اضافه 1\n/نتائج 1\n/اعتماد_نتائج 1\n\n"
        "الترتيب والإحصائيات:\n"
        "/الترتيب_العام\n/ترتيب_نص\n/احصائيات\n/ملف_الاحصائيات 1 39\n\n"
...
async def fantasy_unknown_command(update: Update, context: ContextTypes.DEFAULT_TYPE):
...
def main():
    ...
    app.add_handler(MessageHandler(filters.TEXT & filters.Regex(r"^/(?:إيقاف|ايقاف)_الصور_التلقائية"), admin_only(disable_auto_images)))
    app.add_handler(MessageHandler(filters.TEXT & filters.Regex(r"^/(?:استيراد_ملف|استيراد|استيراد_اكسل|استيراد_إكسل)(?:\s|$)"), admin_only(import_excel_file)))
    app.add_handler(MessageHandler(filters.TEXT & filters.Regex(r"^/اعتماد_استيراد"), admin_only(approve_import)))
    app.add_handler(MessageHandler(filters.TEXT & filters.Regex(r"^/(?:إلغاء|الغاء)_استيراد"), admin_only(cancel_import)))
